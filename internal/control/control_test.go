...
func TestRoleAuthorizer(t *testing.T) {
	a := NewRoleAuthorizer(config.RBACConfig{Roles: []config.Role{
		...
		{Name: "svcadmin", UIDs: []uint32{1100}, Allow: []string{"service.*"}},
	}})
	...
	cases := []struct {
		...
		{"root bootstrap", Peer{UID: 0}, "service.stop", "admin", false},
		{"ops wildcard", Peer{UID: 1000}, "service.stop", "ops", false},
		{"auditor allowed", Peer{UID: 2000, GID: 50}, "status", "auditor", false},
		{"auditor denied op", Peer{UID: 2000, GID: 50}, "service.stop", "", true},
		{"svcadmin prefix", Peer{UID: 1100}, "service.reload", "svcadmin", false},
