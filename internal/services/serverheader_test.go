...
func TestServerStringFor(t *testing.T) {
	cases := map[string]string{
		...
		"Debian":      "nginx/1.22.1",
		"Rocky Linux": "nginx/1.20.1",
		"Fedora":      "nginx/1.24.0 (Fedora Linux)",
...
func TestApplyServerHeader(t *testing.T) {
	...
	// Linux/Rocky → per-distro nginx string; body + Content-Length untouched.
	r := &Responder{options: map[string]string{"os_family": "linux", "os_name": "Rocky Linux"}}
	out := r.applyServerHeader(resp)
	if !strings.Contains(string(out), "Server: nginx/1.20.1\r\n") {
