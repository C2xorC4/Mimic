...
--*/
{
    ...
    do
    {
       ...
       DispatchLevel = NDIS_TEST_SEND_AT_DISPATCH_LEVEL(SendFlags);
        ...
        //
        // ★ mimic-hifi: while armed, rewrite the final IPv4 IP-ID below the Windows IP
        // transmit re-stamp (TCP->0 = nmap TI/CI=Z, ICMP->increment = II=I). Per-protocol,
        // no per-flow state. Edits the in-place buffer when the leading bytes are
        // contiguous (true for the small eth+IP headers we touch); skips the rare
        // copied-fragment case rather than do an MDL write-back.
        //
        if (g_MimicMode == MIMICHIFI_MODE_LINUX)
        {
            PNET_BUFFER_LIST mhNbl;
            for (mhNbl = NetBufferLists; mhNbl != NULL; mhNbl = NET_BUFFER_LIST_NEXT_NBL(mhNbl))
            {
                ...
                for (mhNb = NET_BUFFER_LIST_FIRST_NB(mhNbl); mhNb != NULL; mhNb = NET_BUFFER_NEXT_NB(mhNb))
                {
                    ...
                    if (mhEt != MIMICHIFI_ETHERTYPE_IPV4)
                    {
                        continue;
                    }
                    {
