    ...
    )
{
    ...
    switch (IrpStack->MajorFunction)
    {
        ...
        case IRP_MJ_CLOSE:
            // mimic-hifi fail-safe: disarm when the control handle closes (mimic exit/crash).
            InterlockedExchange(&g_MimicMode, MIMICHIFI_MODE_OFF);
            break;
    ...
    )
{
    ...
    switch (IrpSp->Parameters.DeviceIoControl.IoControlCode)
    {
        ...
        case IOCTL_MIMICHIFI_ARM:
            // arm: 1 input byte = mode (1 = Linux persona). Empty => default Linux.
            InputBuffer = (PUCHAR)Irp->AssociatedIrp.SystemBuffer;
            InputBufferLength = IrpSp->Parameters.DeviceIoControl.InputBufferLength;
            if (InputBuffer != NULL && InputBufferLength >= 1)
            {
                InterlockedExchange(&g_MimicMode, (LONG)InputBuffer[0]);
            }
            else
            {
                InterlockedExchange(&g_MimicMode, MIMICHIFI_MODE_LINUX);
            }
            Status = STATUS_SUCCESS;
            break;

        case IOCTL_MIMICHIFI_DISARM:
            InterlockedExchange(&g_MimicMode, MIMICHIFI_MODE_OFF);
            Status = STATUS_SUCCESS;
            break;
