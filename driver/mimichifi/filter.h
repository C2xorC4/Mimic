...
// mimic-hifi: single flavor (replaces the sample's NDISLWF/NDISMON cascade).
#define FILTER_FRIENDLY_NAME        L"mimic-hifi IP-ID corrector"
#define FILTER_UNIQUE_NAME          L"{B5A2C1F0-9E3D-4A7C-8F21-3D6E5C4B2A19}" // == INF NetCfgInstanceId
#define FILTER_SERVICE_NAME         L"mimichifi"
// Control device link → user mode \\.\MimicHiFi (matches internal/stack/hifi_windows.go).
#define LINKNAME_STRING             L"\\DosDevices\\MimicHiFi"
#define NTDEVICE_STRING             L"\\Device\\MimicHiFi"
